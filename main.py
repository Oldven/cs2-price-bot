...
def get_prices_for_item(item):
    ...
    try:
        rub = requests.get(url_rub, timeout=12).json().get("lowest_price", "Нет")
        usd = requests.get(url_usd, timeout=12).json().get("lowest_price", "Нет")
        return item, rub, usd
    except:
...
def get_all_prices(use_cache=True):
    ...
    # Кэш на 7 минут (чтобы реже делать запросы к Steam)
    if use_cache and cache and cache_time and (now - cache_time).total_seconds() < 420:
        return cache

    # Только 4 одновременных запроса (безопаснее для Steam)
...
def prices(message):
    global user_chat_id
    user_chat_id = message.chat.id
    bot.send_message(message.chat.id, "⏳ Собираю цены (может занять 10–15 сек)...")
...
print("Бот запущен (безопасная версия)!")
bot.polling()
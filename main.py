...
def get_steam_price(item):
    ...
    try:
        return requests.get(url, timeout=15, headers=headers).json().get("lowest_price", "Нет")
    except:
        return "Ошибка"
...
def get_csfloat_price(item):
    ...
    try:
        ...
        if data.get("listings") and len(data["listings"]) > 0:
            price = data["listings"][0].get("price", 0)
            return f"${price}"
        return "Нет"
    except:
        return "Ошибка"
...
def get_lisskins_price(item):
    encoded = quote(item)
    url = f"https://api.lisskins.gg/v1/market/search?q={encoded}&limit=1"
    try:
        data = requests.get(url, timeout=15, headers=headers).json()
